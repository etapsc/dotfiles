...
- For every automatable step, APPEND executable coverage into the GATE-owned fenced block of the per-slice scripts `tests/slices/<slice>-verify.sh` (static/build/lint checks) and `tests/slices/<slice>-smoke.sh` (behavioral checks), where `<slice>` is the literal slice id. Use these EXACT markers (emit verbatim):
  ...
  - Do NOT create or write `tests/e2e/<slice>-manual-automation.sh`. The gate writes ONLY into its own gate-owned fence; the producer (`bridge-coder`/`bridge-debugger`) owns the DISTINCT `producer (managed)` fence in the same files — never touch the producer block, the header, or the trailer.
...
2. Run the per-slice automation for any automatable manual checks: `bash tests/slices/<slice>-verify.sh`, `bash tests/slices/<slice>-smoke.sh` (or `make test-slice SLICE=<slice>`)
