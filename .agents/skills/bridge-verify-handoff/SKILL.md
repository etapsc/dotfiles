---
name: Bridge Verify Handoff
description: (Re)scaffold/refresh the per-slice verification framework (.bridge/ + runners + Makefile wiring) in an existing project so the operator can run make verify from SSH, an iPad, or a phone. Invoke with $bridge-verify-handoff in your prompt.
---
